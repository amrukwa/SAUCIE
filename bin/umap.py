...
def model(X):
    ...
    transformer_umap = UMAP()
    results = transformer_umap.fit_transform(results)
    return transformer_pca, transformer_umap, results
...
transformer_pca, transformer_umap, results = model(X=X)
...
outpath = os.path.join(experiment.get_outputs_path(), 'model_umap.pkl')
with (open(outpath, 'wb')) as outfile:
    pickle.dump(transformer_umap, outfile)

result_path = os.path.join(experiment.get_outputs_path(), 'umap.csv')
